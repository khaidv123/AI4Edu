...
def AI_talk(HISTORY, CURRENT_STAGE):
    ...
    for AI_name in list_AI_name:
        ...
        if external_score < 2.0 and len(HISTORY) > 1:
            ...
            poor_thinking = "Những suy nghĩ gần đây của bạn bị đánh giá kém, vì bạn thực hiện đã những hành động, lời nói LẶP LẠI nhiều lần mà KHÔNG giúp ích cho sự tham gia của các bạn học khác. Bạn cần lập tức thay đổi cách suy nghĩ cho đa dạng, phù hợp hơn."
    ...
    # Chọn 1 người điểm cao nhất, tuy nhiên dựa vào số lượt agent đó bao lấu chưa tương tác (theta)^num ; theta = 1.01 (để sau)
    next_speaker = max(
        evaluator,
    ...
    print(f"========== {next_speaker} talk: ============")
    print(classmate_speak)

    return {"name" : next_speaker,
...
# print("*"*50)
# s = time.time()
# res = AI_talk(HISTORY, CURRENT_STAGE)
# e = time.time()
